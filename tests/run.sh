...
REQUIRED_CHROME_VERSION="Google Chrome 144.0.7559.109"

if [[ -z "${CHROME_PATH:-}" ]]; then
  if [[ -x /usr/bin/google-chrome ]]; then
    export CHROME_PATH=/usr/bin/google-chrome
  else
    printf 'tests: CHROME_PATH must name executable %s\n' "$REQUIRED_CHROME_VERSION" >&2
    exit 1
  fi
fi
...
chrome_version="${chrome_version%"${chrome_version##*[![:space:]]}"}"
if [[ "$chrome_version" != "$REQUIRED_CHROME_VERSION" ]]; then
  printf 'tests: expected %s at CHROME_PATH\n' "$REQUIRED_CHROME_VERSION" >&2
  exit 1
fi
