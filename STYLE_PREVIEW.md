...
本文件用于把“先看后选”的机制接入新 UI 项目：方向不明确时，不靠文字描述争论风格，先产出 3–4 个可看的 mini mockup，再选择正式实现方向。它是静态、自包含的预览协议，不是运行服务。
...
- 第二阶段不提供预览服务器、文件监听器或后台进程，也不要求启动本地服务；直接打开静态 `index.html` 即可。
