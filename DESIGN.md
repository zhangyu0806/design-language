...
### 0.4 页面级 Functional Contract：先说清功能，再决定表现

每个页面在画布局或选组件前，必须用可验证的一句话分别回答：

1. **为什么存在**：这个页面替谁解决什么具体问题？
2. **3 秒内必须知道什么**：用户第一眼要确认自己在哪里、当前状态是什么、接下来能做什么？
3. **必须完成什么动作**：本页唯一主任务是什么，完成后用户会看到什么明确结果？

如果三项答不清，先补信息架构和业务路径，不要用视觉层次掩盖功能空缺。一个页面可以有多个辅助动作，但只能有一个当前主任务；异常时也要给出可继续的下一步。

### 0.5 可选模块路由：按任务加载，不默认膨胀

`DESIGN.md` 是所有项目共享的个人 DNA 核心。以下文件是专项参考，只有任务需要时才加载，未选择时不改变核心规则：

| 模块 ID | 文件 | 何时加载 |
|---|---|---|
| `ui-patterns` | `references/UI_PATTERNS.md` | 设计复杂组件、功能流程、状态机或恢复路径 |
| `motion` | `references/MOTION.md` | 页面含转场、编排、展开、拖拽或重要反馈动效 |
| `data-vis` | `references/DATA_VIS.md` | 页面用图表、指标、趋势、比较或缺失数据表达结论 |
| `preferences` | `references/PREFERENCES.md` | 项目需要记录经证据确认的本地偏好及其治理范围 |

- 模块是补充，不覆盖本文件的 NEVERS、语义 token、preset、三拨盘、可访问性和业务真实性。
- 可同时加载多个模块；只加载与当前任务有关的部分，不自动猜测，不把四个模块默认塞进每个项目。
- `STYLE_PREVIEW.md` 不是可选模块。方向不明确时仍按现有规则默认使用静态风格试衣间。
...
- 检查 `DESIGN.md`、`STYLE_PREVIEW.md`、`presets/*.md`、`references/*.md` 与 `starter/.ai/` 对应副本是否逐字节一致。
...
涉及 UI / 视觉 / 页面重构时，要求 AI 先输出以下执行声明，再写代码：
...
- [ ] Functional contract 三项都有明确答案，主任务和完成结果可验证
- [ ] 没有任何 NEVERS 清单里的东西
...
- [ ] AI 输出或 PR 说明包含 Design read / Functional contract / Design risks / Preflight target / Modules
- [ ] 只加载任务需要的可选模块，模块没有覆盖核心 DNA
