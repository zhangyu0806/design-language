...
### Requirement: Ubuntu 24.04 CI 必须以最小 AppArmor 例外保留浏览器沙箱

Ubuntu 24.04 CI MUST 将 `browser-actions/setup-chrome` 输出经 `readlink -f` 解析，并 MUST 要求结果精确等于 `/opt/hostedtoolcache/setup-chrome/chrome/144.0.7559.109/x64/chrome`。CI MUST 通过 `/etc/apparmor.d/design-language-cft-144` 为该无通配符的唯一可执行路径加载 `flags=(unconfined)` profile，profile MUST 使用 `abi <abi/4.0>`、`include <tunables/global>` 且仅授予 `userns`，并 MUST 通过 `apparmor_parser -r` 加载后才导出 `CHROME_PATH` 和运行测试。版本门禁 MUST 同时精确接受 `Google Chrome 144.0.7559.109` 与 `Google Chrome for Testing 144.0.7559.109`。CI MUST NOT 使用 `--no-sandbox`、setuid helper、全局 AppArmor userns sysctl 降级、禁用 AppArmor 或任何通配/宽路径 profile。

#### Scenario: 固定 CfT 路径与版本都符合预期

- **WHEN** action 输出解析到精确固定路径，AppArmor 专用 profile 成功加载，且浏览器报告两个受支持名称之一和精确版本
- **THEN** CI 把该解析路径导出到 `GITHUB_ENV`，并以保持 sandbox 的真实 Chrome 运行全部浏览器测试

#### Scenario: action 输出路径漂移

- **WHEN** `readlink -f` 的结果不精确等于固定 CfT 144 可执行路径
- **THEN** CI 在写入 AppArmor profile、导出 `CHROME_PATH` 或启动浏览器前失败，不用通配路径放宽规则

#### Scenario: 安全例外无法加载

- **WHEN** 精确路径 AppArmor `userns` profile 无法由 `apparmor_parser -r` 加载
- **THEN** CI 失败，且不以禁用 sandbox、setuid helper 或全局安全策略降级继续运行
